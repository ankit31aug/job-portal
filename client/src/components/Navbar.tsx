        ...
        <div className="flex justify-between h-16 items-center">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-gray-900 dark:text-white">
              ...
              <span className="text-brand-500">{settings.site_name.split(' ').slice(-1)}</span>
            </span>
          </Link>

          <div className="hidden md:flex items-center gap-6">
            <Link to="/browse" className="text-gray-600 dark:text-gray-300 hover:text-brand-500 dark:hover:text-white font-medium transition-colors">Browse Jobs</Link>
            <Link to="/careers" className="text-gray-600 dark:text-gray-300 hover:text-brand-500 dark:hover:text-white font-medium transition-colors flex items-center gap-1">
              <TrendingUp size={16} />Career Paths
            </Link>
            <Link to="/about" className="text-gray-600 dark:text-gray-300 hover:text-brand-500 dark:hover:text-white font-medium transition-colors">About QCI</Link>
            <Link to="/resume-match" className="text-gray-600 dark:text-gray-300 hover:text-brand-500 dark:hover:text-white font-medium transition-colors flex items-center gap-1">
              <FileText size={16} />Resume Match
            </Link>
            {user?.role === 'employer' && (
              <Link to="/post-job" className="text-gray-600 dark:text-gray-300 hover:text-brand-500 dark:hover:text-white font-medium transition-colors flex items-center gap-1">
          ...
          <div className="md:hidden py-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 space-y-1">
            ...
            {user ? (
              ...
              <>
                ...
                <Link to="/register" onClick={() => setMenuOpen(false)} className="block px-4 py-2 text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/20 rounded-lg font-medium">Get Started</Link>
              </>
            )}
          </div>
        )}
